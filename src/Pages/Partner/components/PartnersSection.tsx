...
function PartnerTab({ partner, isActive, onClick }: PartnerTabProps) {
  ...
  return (
    <button onClick={onClick} style={{ width: "100%", textAlign: "left", border: "none", cursor: "pointer", background: "none", padding: 0 }}>
      ...
      >
        <div style={{ width: 38, height: 38, borderRadius: 10, background: isActive ? color : B.bg, display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0, transition: "all .3s" }}>
          <span style={{ fontSize: 12, fontWeight: 800, color: isActive ? "#fff" : B.subtle, letterSpacing: ".05em" }}>{num}</span>
        </div>
        <div style={{ flex: 1, minWidth: 0 }}>
          <p className="selector-label" style={{ fontSize: 10, fontWeight: 700, letterSpacing: ".14em", textTransform: "uppercase", color: isActive ? color : B.subtle, marginBottom: 2, transition: "color .3s" }}>
            {label}
          </p>
          <p style={{ fontSize: 13.5, fontWeight: 700, color: B.text, lineHeight: 1.25 }}>{headline}</p>
        </div>
...
function PartnerDetail({ partner, formRef }: PartnerDetailProps) {
  const { label, headline, desc, offers, models, color, colorLight, colorMid, icon } = partner;
    ...
    <div style={{ borderRadius: 22, overflow: "hidden", border: `1.5px solid ${colorMid}`, boxShadow: `0 12px 48px ${color}15` }}>
      <div style={{ background: color, padding: "28px 32px", display: "flex", alignItems: "flex-start", justifyContent: "space-between", gap: 16 }}>
        <div>
          ...
          <h3 style={{ fontFamily: "'Lora',serif", fontSize: "clamp(1.2rem,2vw,1.6rem)", fontWeight: 600, color: "#fff", lineHeight: 1.25 }}>{headline}</h3>
        </div>
        <div style={{ width: 52, height: 52, borderRadius: 14, background: "rgba(255,255,255,0.15)", display: "flex", alignItems: "center", justifyContent: "center", color: "#fff", flexShrink: 0 }}>{icon}</div>
      </div>

      <div style={{ background: B.white, padding: "28px 32px" }}>
        <p style={{ fontSize: 14, color: B.muted, lineHeight: 1.82, marginBottom: 24 }}>{desc}</p>
        ...
        <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "10px 16px", marginBottom: 24 }}>
          {offers.map((offer, i) => (
            <div key={i} style={{ display: "flex", alignItems: "flex-start", gap: 10 }}>
              ...
              <span style={{ fontSize: 13, color: B.text, lineHeight: 1.5 }}>{offer}</span>
            </div>
          ))}
        </div>
        ...
        {models.length > 0 && (
          <>
            ...
            <div style={{ display: "flex", flexWrap: "wrap", gap: 8, marginBottom: 24 }}>
              {models.map((model, i) => (
                <span key={i} style={{ fontSize: 12, fontWeight: 600, padding: "6px 13px", borderRadius: 99, background: colorLight, color, border: `1px solid ${colorMid}` }}>
                  {model}
                </span>
              ))}
            </div>
        ...
        <button
          ...
          style={{ display: "flex", alignItems: "center", gap: 8, background: color, color: "#fff", border: "none", borderRadius: 11, padding: "12px 22px", fontSize: 13, fontWeight: 700, cursor: "pointer", boxShadow: `0 6px 20px ${color}30`, transition: "all .2s" }}
          onMouseEnter={(event) => {
            event.currentTarget.style.opacity = ".88";
          }}
          onMouseLeave={(event) => {
      ...
      <div style={{ maxWidth: 1200, margin: "0 auto" }}>
        <div ref={ref} className="reveal" style={{ display: "flex", alignItems: "flex-end", justifyContent: "space-between", flexWrap: "wrap", gap: 20, marginBottom: 52 }}>
          <div>
            ...
            <h2 style={{ fontFamily: "'Lora',serif", fontSize: "clamp(2rem,3.5vw,2.8rem)", fontWeight: 600, color: B.dark, lineHeight: 1.1 }}>
              A partnership for
              every stakeholder
            </h2>
          </div>
